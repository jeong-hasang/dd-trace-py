...
class CodeFunctionCache
{
  public:
    ...
    /* Inserts (code, id, lasti, line). If the target set is full, evicts the
     * least recently used way via CLOCK. */
  ...
  private:
    struct Set
    {
        ...
        int lastis[WAYS_PER_SET] = { -1, -1, -1, -1 }; // -1 = no lasti cached
        int lines[WAYS_PER_SET] = { 0, 0, 0, 0 };
